...
################################################################################
#             New script for generating  refinance offers upon payment         #
...
if(nrow(select)==0){
  output <- as.data.frame(cbind(application_id,NA))
  names(output) <- c("id","score")
} else {
  output <- as.data.frame(cbind(application_id,select$max_amount))
  names(output) <- c("id","score")
}


# Read and write
final_exists <- read.xlsx(paste(main_dir,"Scored_Credits_Refinance.xlsx", 
                                sep=""))
final <- rbind(final_exists, output)
write.xlsx(final, paste(main_dir,"Scored_Credits_Refinance.xlsx", sep=""))
