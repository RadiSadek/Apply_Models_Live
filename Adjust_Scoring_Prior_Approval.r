  ...
  if(nrow(po)>=1){
    ...
    if(nrow(po)>=1){
      ...
      if(nrow(all_id_local)==0 & po$final_time<=4){
        ...
        period_po <- suppressWarnings(dbFetch(dbSendQuery(con, 
...
gen_correction_po_ref <- function(con,db_name,all_df,all_id,
  ...
  if(nrow(input)>0){
    ...
    # Read credits with already an offer for terminated prior approval
    po_ref <- suppressWarnings(dbFetch(dbSendQuery(con,
...
gen_corection_early_repaid <- function(con,db_name,scoring_df,all_df,all_id,
  ...
  tot_installments <-suppressWarnings(dbFetch(dbSendQuery(con,
    gen_last_cred_amount_query(
    all_id_here$id,db_name))))$installments
  passed_installments <- suppressWarnings(dbFetch(dbSendQuery(con,
  ...
  period <- suppressWarnings(dbFetch(dbSendQuery(con,
